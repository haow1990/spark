...
object LDAExample {
  ...
  private def run(params: Params) {
    ...
    val lengthStat = corpus.map(_._2.indices.length).map((_, 1)).reduceByKey(_+_).collect.mkString("\n")
    println(s"doc terms stat: \n$lengthStat")
...
//      .setCheckpointInterval(params.checkpointInterval)
    ...
    val ldaModel = LDA.train(corpus, params.k, params.maxIterations)
...
//    }
  ...
  private def preprocess(
      ...
      paths: Seq[String],
      vocabSize: Int,
      stopwordFile: String): (RDD[(Long, SparseVector)], Long, Array[String], Long) = {
    val partitions = sc.getConf.getInt("spark.lda.partitions", -1)

    // Get dataset of document texts
    // One document per line in each text file.
    val textRDD: RDD[String] =
      if (partitions > 0)
        sc.textFile(paths.mkString(","), partitions)
      else
        sc.textFile(paths.mkString(","))
    ...
    (documents, docSize, vocabArray, selectedTokenCount)
  }
}
